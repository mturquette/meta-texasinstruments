...
SRC_URI = "\
	file://23.13-aacencnocore.patch;patch=1 \
	file://23.13-aacencnoincinstall.patch;patch=1 \
	file://23.13-aacenctestnocore.patch;patch=1 \
	"
...
do_compile() {
	cd ${S}/audio/src/openmax_il/aac_enc
	oe_runmake \
		PREFIX=${D} PKGDIR=${S} \
		CROSS=${AR%-*}- \
...
do_install_cleanup() {
	mv ${D}/omx/audio_encoder.aac* ${D}/omx/patterns
}
